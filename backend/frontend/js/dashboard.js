...
// Quick category
function selectCategory(cat){ document.getElementById("category").value = cat; }
...
async function addExpense() {
  ...
  try {
    const res = await fetch(`${BASE_URL}/api/expenses`,{
      method:"POST", headers:authHeaders, body:JSON.stringify({category,amount,note})
    });
    if(!res.ok) return res.status===401?logout():showToast("Failed to add 😅","error");
...
async function loadExpenses() {
  try {
    ...
    displayExpenses(); updateCalendarEvents();
...
function displayExpenses(arr=expenses){
  ...
  const table = document.getElementById("expenseTable"); table.innerHTML="";
  let categoryTotals={}, now=new Date(), today=0, week=0, month=0;
  ...
  updateChart(categoryTotals); updateBudget(month);
}

// Delete Expense
async function deleteExpense(id){
  try{
    const res=await fetch(`${BASE_URL}/api/expenses/${id}`,{method:"DELETE",headers:authHeaders});
...
function filterExpenses(){
  ...
  displayExpenses(expenses.filter(e=>e.category.toLowerCase().includes(t)));
}
...
function setBudget(){
  ...
  localStorage.setItem("budget",b); updateBudget(0);
}
function updateBudget(spent){
  const budget=Number(localStorage.getItem("budget"));
  if(!budget) return;
  document.getElementById("budgetStatus").innerText = spent>budget?"⚠ Budget exceeded!":"Remaining ₹"+(budget-spent);
}
...
function downloadPDF(){
  ...
  const doc=new jsPDF();
  doc.setFont("Helvetica"); doc.setFontSize(18); doc.text("Expense Report",20,20);
  doc.setFontSize(12); let y=40;
  expenses.forEach(e=>{doc.text(`${e.category} - ₹${e.amount}`,20,y);y+=10;});
...
function updateCalendarEvents(){
  if(!calendar) return;
  const daily={};
  expenses.forEach(e=>{ const d=e.date.split("T")[0]; daily[d]=(daily[d]||0)+Number(e.amount); });
  const events=Object.keys(daily).map(d=>({title:"₹"+daily[d],start:d}));
